export async function onRequest(context) {
  ...
  var airtableHeaders = { Authorization: 'Bearer ' + PAT, 'Content-Type': 'application/json' };
  ...
  try {
    ...
    if (!username || !email) {
      return new Response(JSON.stringify({ error: 'Missing username or email' }), { status: 400, headers: corsHeaders });
    }

    var checkRes = await fetch(BASE_URL + '/Users?filterByFormula=' + encodeURIComponent('{Username}="' + username + '"') + '&maxRecords=1', { headers: airtableHeaders });
    var checkData = await checkRes.json();
    if (checkData.records && checkData.records.length > 0) {
      return new Response(JSON.stringify({ error: 'Username already taken' }), { status: 400, headers: corsHeaders });
    }

    var countRes = await fetch(BASE_URL + '/Users?pageSize=100&fields%5B%5D=Username', { headers: airtableHeaders });
    ...
    if (!result.records || result.records.length === 0) {
      return new Response(JSON.stringify({ error: 'Failed to create user', detail: JSON.stringify(result) }), { status: 500, headers: corsHeaders });
    }
    ...
    if (isFounding) {
      var badgeRes = await fetch(BASE_URL + '/Badges?filterByFormula=' + encodeURIComponent('{Name}="FOUNDING 100"') + '&maxRecords=1', { headers: airtableHeaders });
    ...
    return new Response(JSON.stringify({ error: 'Server error', message: err.message }), { status: 500, headers: corsHeaders });
  }
}
